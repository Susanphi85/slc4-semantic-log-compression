...
function bitunpack(buf,n,bits){const vals=[];let bitpos=0;for(let k=0;k<n;k++){let v=0;for(let j=0;j<bits;j++){const idx=bitpos+j;if(buf[Math.floor(idx/8)]&(1<<(idx%8)))v|=1<<j;}vals.push(v);bitpos+=bits;}return vals;}
...
function zstdCompress(data,level=1){return zstd.compress(data,level);}
...
function decodeRuns(blob,runs,valueDecoder){const vals=[];let pos=0;for(let r=0;r<runs;r++){let x=decUvarint(blob,pos);const count=Number(x[0]);pos=x[1];x=decUvarint(blob,pos);const ln=Number(x[0]);pos=x[1];const vb=blob.subarray(pos,pos+ln);pos+=ln;const v=valueDecoder(vb);for(let i=0;i<count;i++)vals.push(v);}return vals;}
...
function decodeNumericTemplate(meta,blob){const n=meta.n;let pos=0,x=decUvarint(blob,pos);const mlen=Number(x[0]);pos=x[1];const mb=blob.subarray(pos,pos+mlen);pos+=mlen;const mant=decodeIntStream(mb,n,meta.menc);let scales;if(meta.sdict){const idx=bitunpack(blob.subarray(pos),n,meta.sbits);scales=idx.map(i=>meta.sdict[i]);}else{scales=[];for(let i=0;i<n;i++){x=decUvarint(blob,pos);scales.push(Number(x[0]));pos=x[1];}}
...
function decodeStringDictValues(meta,blob){const enc=meta.denc,n=meta.n;if(enc==='raw'||enc==='prefix'){const vals=[];let pos=0,p=meta.prefix||'';for(let i=0;i<n;i++){const x=decUvarint(blob,pos),ln=Number(x[0]);pos=x[1];vals.push(p+blob.toString('utf8',pos,pos+ln));pos+=ln;}return vals;}if(enc==='front')return decodeFrontStrings(blob,n);if(enc==='hex'||enc==='prefix_hex'){const w=meta.hexchars/2,p=meta.prefix||'';const vals=[];for(let i=0;i<n;i++){let v=blob.subarray(i*w,(i+1)*w).toString('hex');if(meta.hcase==='upper')v=v.toUpperCase();vals.push(p+v);}return vals;}throw new Error(enc);}
...
function decodeColumn(meta,blob){const enc=meta.enc,n=meta.n;if(enc==='empty')return[];if(enc==='const')return Array.from({length:n},()=>meta.value);if(enc==='boolbits')return bitunpack(blob,n,1).map(Boolean);if(['ints','intdelta','intdelta2'].includes(enc))return decodeIntStream(blob,n,enc).map(Number);if(enc==='intdict'){let x=decUvarint(blob,0),dlen=Number(x[0]),pos=x[1];const db=blob.subarray(pos,pos+dlen);pos+=dlen;const dvals=decodeIntStream(db,meta.dcount,'ints').map(Number);return bitunpack(blob.subarray(pos),n,meta.bits).map(i=>dvals[i]);}if(enc==='intrle')return decodeRuns(blob,meta.runs,b=>Number(unzigzag(decUvarint(b,0)[0])));if(enc==='timestamp'){let x=decUvarint(blob,0),ln=Number(x[0]),pos=x[1];const ib=blob.subarray(pos,pos+ln);pos+=ln;const ns=decodeIntStream(ib,n,meta.tmode),idx=bitunpack(blob.subarray(pos),n,meta.pbits),prec=idx.map(i=>meta.pdict[i]);return ns.map((v,i)=>nsToTs(v,prec[i]));}if(enc==='uintstr')return decodeIntStream(blob,n,meta.umode).map(String);if(enc==='numtemplate')return decodeNumericTemplate(meta,blob);if(enc==='ipv4'){const vals=[];for(let i=0;i<n;i++)vals.push([...blob.subarray(i*4,i*4+4)].join('.'));return vals;}if(enc==='strdict'){let x=decUvarint(blob,0),dlen=Number(x[0]),pos=x[1];const db=blob.subarray(pos,pos+dlen);pos+=dlen;const dvals=decodeStringDictValues(meta.dmeta,db);return bitunpack(blob.subarray(pos),n,meta.bits).map(i=>dvals[i]);}if(enc==='strrle')return decodeRuns(blob,meta.runs,b=>b.toString('utf8'));if(enc==='hex'||enc==='prefix_hex'){const w=meta.hexchars/2,p=meta.prefix||'',vals=[];for(let i=0;i<n;i++){let v=blob.subarray(i*w,(i+1)*w).toString('hex');if(meta.hcase==='upper')v=v.toUpperCase();vals.push(p+v);}return vals;}if(enc==='prefix_raw'||enc==='strraw'){const vals=[];let pos=0,p=meta.prefix||'';for(let i=0;i<n;i++){const x=decUvarint(blob,pos),ln=Number(x[0]);pos=x[1];vals.push(p+blob.toString('utf8',pos,pos+ln));pos+=ln;}return vals;}if(enc==='front')return decodeFrontStrings(blob,n);if(enc==='msgpack')return msgpack.decode(blob);throw new Error(`unknown encoding ${enc}`);}
...
function decodeSchemaSeq(meta,blob){if(meta.enc==='bitpack')return bitunpack(blob,meta.n,meta.bits);if(meta.enc==='rle')return decodeRuns(blob,meta.runs,b=>Number(decUvarint(b,0)[0]));throw new Error(meta.enc);}
...
function decode(buf0){const buf=Buffer.from(buf0);if(!buf.subarray(0,4).equals(MAGIC))throw new Error('Not SLC4');const ml=buf.readUInt32BE(4),meta=msgpack.decode(buf.subarray(8,8+ml));let pos=8+ml,sl=meta.schema_seq_len;const sblob=buf.subarray(pos,pos+sl);pos+=sl;const data=buf.subarray(pos),sseq=decodeSchemaSeq(meta.schema_seq,sblob),paths=meta.paths,cols=[];for(const cm of meta.columns)cols.push(decodeColumn(cm,data.subarray(cm.offset,cm.offset+cm.length)));const curs=Array(paths.length).fill(0),out=[];for(const sid of sseq){const rec=Object.create(null);for(const pid of meta.schemas[sid]){const v=cols[pid][curs[pid]++];setNested(rec,paths[pid],v);}out.push(rec);}return out;}
function inspect(buf0){const buf=Buffer.from(buf0);if(!buf.subarray(0,4).equals(MAGIC))throw new Error('Not SLC4');const ml=buf.readUInt32BE(4);return msgpack.decode(buf.subarray(8,8+ml));}
module.exports={MAGIC,setSelectionMode,encode,decode,inspect,zstdCompress,flattenLeaves};
